...
- ✅ Auto-embedding on upload (no manual "Embed" button needed)

### Day 4+ - Conversation-Scoped RAG & Data Management
- ✅ **Conversation model**: Each chat has its own context
- ✅ **Scoped uploads**: Documents belong to specific conversations
- ✅ **Scoped queries**: Answers only use files from the current chat
- ✅ **Cascading deletes**: Remove a chat → removes all its files, chunks, and embeddings
- ✅ **Sidebar with chat list**: Create, select, and delete conversations
- ✅ **Multi-file upload**: Up to 10 PDFs at once with auto-embedding

### Streaming Responses
- ✅ **Token-by-token streaming**: Responses appear as they're generated by Ollama
- ✅ **Stop generation**: Click "Stop generating" button to cancel mid-stream
...
4. Click the 🗑️ (trash) icon to delete a conversation
...
5. Files are uploaded sequentially and **automatically embedded**:
   ...
   - Success toast shows: `Uploaded filename.pdf — embedded ✓ (X chunks)`
